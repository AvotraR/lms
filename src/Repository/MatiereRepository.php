...
class MatiereRepository extends ServiceEntityRepository
{
    ...
    public function search($recherche){
        ...
        if ($recherche->getPrixMin()){
            $query = $query->andWhere('m.PrixMat >'. $recherche->getPrixMin());
        }
        if ($recherche->getPrixMax()){
            $query = $query->andWhere('m.PrixMat >'.$recherche->getPrixMax());
        }
        if ($recherche->getCategorie()){
            $query = $query->join('m.categorie', 'c')
                           ->andWhere('c.id IN (:Categorie)')
                           ->setParameter('Categorie',$recherche->getCategorie());
        }
